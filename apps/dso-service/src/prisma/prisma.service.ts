...
export class PrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor() {
    const { PrismaClient } = require('.prisma/dso-client');
  ...
  async onModuleInit() {
    ...
    this.logger.log('Database connected (dso schema)');
  ...
  private async createHypertables(): Promise<void> {
    try {
      ...
      await this.ensureHypertable('energy_snapshots', 'timestamp');
      await this.ensureHypertable('dso_optimization_logs', 'time');
    } catch (error) {
      this.logger.warn(
